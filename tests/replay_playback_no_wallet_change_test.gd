...
func run() -> void:
	...
	assert(playback_source.find("connect_to_server") == -1)
	assert(playback_source.find("apply_wallet") == -1)
	assert(playback_source.find("spend_gems") == -1)
	assert(playback_source.find("ProfileServiceScript") == -1)
