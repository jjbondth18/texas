...
func _init() -> void:
	...
	_assert(source.find("active_players.sort_custom(func(a, b): return _seat_order_rank") != -1, "PlayerStatus must sort by seat index, not turn order.")
	_assert(source.find("return seat_index if seat_index > 0 else 999") != -1, "PlayerStatus rank must be ascending seat_index.")
	_assert(source.find("TABLE_SEAT_JOIN_ORDER_9P") == -1, "PlayerStatus must not use table join order.")
