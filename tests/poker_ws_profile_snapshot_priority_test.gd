...
func _init() -> void:
	...
	_require(protocol_source.find("RENAME_DISPLAY_NAME") != -1, "protocol should define rename_display_name")
	_require(protocol_source.find("{\"display_name\": display_name}") != -1, "rename request should send only the game display name")
	_require(source.find("func rename_display_name(display_name: String)") != -1, "websocket client should expose authoritative rename")
	_require(source.find("if resolved_provider == \"steam\"") != -1, "Steam hello should send the current persona instead of a stale local display name")
