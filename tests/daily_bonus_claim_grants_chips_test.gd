...
func _init() -> void:
	...
	_require(PlayerProfileScript.get_total_chips(after) == PlayerProfileScript.get_total_chips(before) + int(reward.get("chips", 0)), "claim should grant day 1 chips.")
	ProfileServiceScript.reset_mock_profile()
	print("Daily bonus claim grants chips test passed.")
	quit(0)
