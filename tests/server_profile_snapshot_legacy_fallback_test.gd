...
func _init() -> void:
	...
	_require(not bool(legacy.get("mock_purchase_allowed", true)), "legacy server response should default mock purchase permission to false")
	var previous_xp := int(legacy.get("total_xp", 0))
	var previous_hands := int(legacy.get("total_hands_played", 0))
	var previous_steam_persona := String(legacy.get("steam_persona_name", ""))
	...
	_require(not bool(partial.get("mock_purchase_allowed", true)), "partial legacy snapshot should keep mock purchase disabled")
	_require(String(partial.get("steam_persona_name", "")) == previous_steam_persona, "legacy snapshot should not clear missing Steam persona")
	_require(int(partial.get("schema_version", 0)) == PlayerProfileScript.SCHEMA_VERSION, "legacy profile should migrate to current nickname schema")
	ProfileServiceScript.reset_mock_profile()
