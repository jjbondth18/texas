...
func _init() -> void:
	...
	_require(str(joined.get("table_id", "")) == "gem_match", "Quick Gem must match an existing public_gem table with the same config.")
	PublicTableRegistryScript.reset()
	print("Quick Gem matches public gem table test passed.")
	quit(0)
