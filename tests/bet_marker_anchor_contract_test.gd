...
func _init() -> void:
	...
	_assert(source.find("const BET_MARKER_ANCHORS_BY_SEAT := BET_MARKER_OFFSETS") != -1, "BetMarker anchors must be a seat-index contract.")
	for seat_id in range(1, 10):
		_assert(source.find("%d: Vector2" % seat_id) != -1, "BetMarker anchor missing for seat %d." % seat_id)
	_assert(source.find("return card_pos + _bet_marker_offset()") != -1, "BetMarker position must use a fixed seat offset.")
